...
// 入口
func (this *WebSpider) Fetch(url, targetPath string) {
...
// go routine do it
func (this *WebSpider) goDo(url string, needException bool) {
	// this.do(url, false)
	this.w.Add(1)
...
// 返回一个[]string 待处理的子
func (this *WebSpider) do(url string, needException bool) (children []string) {
	children = nil
...
// 处理css
func (this *WebSpider) doCSS(url, content string) (children []string) {
	children = nil
	// 保存该文件
	if !this.writeFile(url, content) {
		return
...
// 生成子的相对目录有用
func (this *WebSpider) doHTML(pUrl, realPUrl, content string) (children []string) {
	regular := "(?i)(src=|href=)[\"']([^#].*?)[\"']"
	reg := regexp.MustCompile(regular)
	re := reg.FindAllStringSubmatch(content, -1)
...
// cUrl a.com/c/d/e/g
// 在realPUrl页面到cUrl跳转
func (this *WebSpider) getRalativeUrl(realPUrl, cUrl string) (url string, ok int) {
	ok = 0
	url = cUrl
...
// trimSpace, /, \, ", '
func (this *WebSpider) trimUrl(url string) string {
	if url != "" {
		url = strings.TrimSpace(url)
...
// 处理异常
func (this *WebSpider) doExceptionUrl() {
...
// 如果不是一个目录, 返回""
func (this *WebSpider) genFilename(url string) (string, bool) {
...
// url = a.com/a/?id=12&id=1221, 那么genUrl=a.com/a/index.html?id=121
func (this *WebSpider) genUrl(url string) string {
	// 去掉?后面的
...
func (this *WebSpider) writeFile(url, content string) bool {
	// $path = a.html?a=a11
...
func (this *WebSpider) cleanUrl(url string) string {
	url = filepath.Clean(url)
	return strings.Replace(url, "\\", "/", -1)
}

// 将url ?, #后面的字符串去掉
func (this *WebSpider) trimQueryParams(url string) string {
	pos := strings.Index(url, "?")
	if pos != -1 {
		url = util.Substr(url, 0, pos)
	}
...
// 判断是否已存在
// url = a/b/c/d.html
func (this *WebSpider) isExists(url string) bool {
	return util.IsExists(this.targetPath + "/" + url)
}

// 不需要处理的url
// needException false 表示不要处理, 那么就要判断是否在其中
func (this *WebSpider) isNotNeedUrl(url string, needException bool) bool {
...
// 处理url, 得到scheme, host
func (this *WebSpider) parseUrl(url string) {
...
func (this *WebSpider) getNoChildrenFileExts() []string {
	return this.noChildrenFileExts
}

// 得到内容
func (this *WebSpider) getContent(url string) (content string, err error) {
...
// 生成存储位置
func (this *WebSpider) doTargetPath(path string) {
...
func (this *WebSpider) setLogOutputWriter() {
